...
# Read files
	uncertainties = json.loads(filecontent("%s/vbfHbb_uncertainties_2015_run2.json"%basepath))
...
#	for j,jmass in enumerate(range(115,140,5)):
 ...
 ## top info
		...
		fout.write("%-15s"%"rate")
		for i in CATS:
			fout.write("%-8s"%("%.2f"%yVBF[i][j]))
			fout.write("%-8s"%("%.2f"%yGF[i][j]))
			fout.write("%-8d"%(yD[i]))
			fout.write("%-8s"%("%.2f"%yT[i]))
			fout.write("%-8s"%("%.2f"%yZ[i]))
		fout.write("\n"+"-"*100+"\n")
  ## fixed parts
