...
#
	mp.add_option('--workdir',help=colours[5]+'Case workdir.'+colours[0],default='case_new5jet_swap_doub_3cat_test',type='str')
...
#
	...
	mg1.add_option('--SELCATs',help=colours[5]+'Selection/Category setup: "Single;SingleB;-0.5#0.31#0.59#0.67#0.7325,Double;DoubleB;0.12#0.44#0.57#.6725,...".'+colours[0],default='Single;SingleB;-0.5#0.31#0.59#0.67#0.7325,Double;DoubleB;0.12#0.44#0.57#.6725',type='str',action='callback',callback=SELsetup,dest='SC')# final
...
#
	...
	mg2.add_option('--TF',help=colours[5]+'Transfer function label: POL1,POL2 (Single,Double)'+colours[0],default=['POL1','POL2'],type='str',action='callback',callback=optsplit)
	...
	mg2.add_option('--binsize',help=colours[5]+'Transfer function bin size: 10,5 (Single,Double)'+colours[0],default=[10,5],type='str',action='callback',callback=optsplitint,dest='BINS')
...
def scale(tf,cat):
	s = { "POL1":[1.0,0.5,0.67,0.67,1.,1.2,1.7],
...
# Setup
	...
	TF = opts.TF
	TFinfo = json.loads(filecontent("%s/vbfHbb_transfer_2015_run2.json"%basepath))
...
## CMS info
	...
	pCMS2.AddText("L = 2.19 fb^{-1} (13 TeV)")
...
## Load trees
	#	fin = TFile.Open("flat/Fit_data_sel%s.root"%S.tag,"read")
		fin = TFile.Open("/afs/cern.ch/work/n/nchernya/Hbb/Combine/toolkit/flat/run2/root/Fit_BTagCSV_golden_sel%s.root"%S.tag,"read")
...
#### Other CATs
  ...
  ## Mbb plot
				...
				h.SetMaximum(.3 if iS==0 else 0.2)
				h.Draw("hist")
