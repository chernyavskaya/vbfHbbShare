...
#	finfo  = json.loads(filecontent("%s/vbfHbb_info_2013.json"%basepath))
	fnames = json.loads(filecontent("%s/vbfHbb_samples_2015_run2_sig.json"%basepath))
	finfo  = json.loads(filecontent("%s/vbfHbb_info_2015_run2.json"%basepath))
...
#
 ...
 ## File reading
	...
	for f in [x['fname'] for x in fnames['files'].itervalues()]:
		...
		styp = re.search('.*sel(Single|Double).*',f).group(1)
	...
	#	inf[mass][styp][ftyp] = TFile.Open("flat/"+f,'read')
		inf[mass][styp][ftyp]= TFile.Open("/afs/cern.ch/work/n/nchernya/Hbb/Combine/toolkit/flat/run2/root/"+f,'read')
...
### Category loop
  ...
  #### Define cut
				cut = TCut("puWt*trigWt%s*(mva%s>%.2f && mva%s<=%.2f)"%(S.tag,S.tag,S.boundaries[C],S.tag,S.boundaries[C+1]))
  #### Draw mass
